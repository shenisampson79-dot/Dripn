...
assert.equal(APPLE_AI_TOPUP_PRODUCT_IDS.standard, 'com.dripn.ai.topup.300');
assert.equal(APPLE_AI_TOPUP_PRODUCT_IDS.plus, 'com.dripn.ai.topup.600');

assert.equal(creditsForAiTopUpProductId('com.dripn.ai.topup.300'), 300);
assert.equal(displayNameForAiTopUpProductId('com.dripn.ai.topup.300'), 'AI Top-Up');
...
const standard = resolveAiTopUpFromProductId('com.dripn.ai.topup.300');
