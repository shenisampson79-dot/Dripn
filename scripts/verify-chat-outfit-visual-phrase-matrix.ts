...
const PHRASE_MATRIX = [
  ...
  'Give me an outfit for work today',
] as const;
...
const workToday = resolveOutfitRoute({
  userText: 'Give me an outfit for work today',
  messages: [],
  wardrobeItems: [],
});
assert.equal(workToday.route, 'outfit-from-wardrobe', 'work-today must use dedicated outfit route');
