...
- **Profile Persistence**: Full onboarding profile (gender, skinUndertone, bodyMeasurements, extendedPreferences, stylistPreferences, etc.) is persisted to the `users.profile_data` JSONB column via `PUT /api/auth/profile/sync`. `GET /api/auth/me` returns `profileData`. On login/social login, if local AsyncStorage has no onboarding data, the profile is automatically restored from the backend — survives reinstalls and device changes. `SkinUndertone` is now a proper exported type in `AuthContext.tsx` and is included in `UserProfile` and passed in `completeOnboarding`.
